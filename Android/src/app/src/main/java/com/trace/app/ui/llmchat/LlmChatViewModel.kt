...
package com.trace.app.ui.llmchat
...
import com.trace.app.common.SystemPromptHelper
...
import com.trace.app.ui.common.chat.ChatMessageLoading
import com.trace.app.ui.common.chat.ChatMessageText
import com.trace.app.ui.common.chat.ChatMessageFile
import com.trace.app.ui.common.chat.ChatMessageThinking
import com.trace.app.ui.common.chat.ChatMessageType
import com.trace.app.ui.common.chat.ChatMessageWarning
import com.trace.app.ui.common.chat.ChatSide
import com.trace.app.ui.common.chat.ChatViewModel
...
import com.trace.app.voice.InteractionOrigin
...
open class LlmChatViewModelBase(
  ...
  // query one index.
  private val sharedRagEngine: com.trace.app.rag.RagEngine? = null,
  ...
  private val memoryRepository: com.trace.app.memory.MemoryRepository? = null,
  private val notificationScheduleManager:
    com.trace.app.notifications.NotificationScheduleManager? = null,
  ...
  private var ttsManager: com.trace.app.voice.TtsManager? = null

  // --- Phase 3 RAG ---
  private val ragEngine: com.trace.app.rag.RagEngine?
    get() = sharedRagEngine
  ...
  private val _ragResponse =
    MutableStateFlow<com.trace.app.rag.RagResponse?>(null)
  ...
  private val scheduleExtractor:
    com.trace.app.memory.ScheduleExtractor? by lazy {
    val mem = memoryRepository
    val sched = notificationScheduleManager
    if (mem != null && sched != null) {
      com.trace.app.memory.ScheduleExtractor(mem, sched)
    } else null
  }
   ...
   * [com.trace.app.memory.ScheduleExtractor]). This is the
  ...
  fun createReminderFromRequest(
    ...
    documentText: String = "",
    source: com.trace.app.proto.MemorySource =
      com.trace.app.proto.MemorySource.MEMORY_SOURCE_VISION_PRESCRIPTION,
    onResult: (com.trace.app.memory.ScheduleExtractor.Result) -> Unit = {},
  ) {
    val extractor =
      scheduleExtractor
        ?: run {
          onResult(
            com.trace.app.memory.ScheduleExtractor.Result.Failed(
  ...
  fun initTts(context: android.content.Context) {
      if (ttsManager == null) {
          ttsManager = com.trace.app.voice.TtsManager(context)
      }
  }
  ...
  override fun onCleared() {
      ...
      com.trace.app.filefetch.SemanticFileMatcher.clearClassifier()
  ...
  fun registerSemanticClassifier(model: Model) {
      com.trace.app.filefetch.SemanticFileMatcher.setClassifier { bitmap, prompt ->
  ...
  fun tryHandleQuickTools(
    ...
    val calc = com.trace.app.tools.CalculatorTool.tryHandle(input)
    val answer: String =
      when (calc) {
        is com.trace.app.tools.CalculatorTool.Result.Success ->
          "${calc.expression} = ${com.trace.app.tools.CalculatorTool.formatValue(calc.value)}"
        is com.trace.app.tools.CalculatorTool.Result.Error ->
          "Can't compute \"${calc.expression}\" — ${calc.reason}."
        null ->
          if (com.trace.app.tools.DateTimeTool.isDateTimeQuery(input)) {
            com.trace.app.tools.DateTimeTool.answer()
          } else {
            return false
          }
      }
  ...
  fun tryHandleWebSearch(
    ...
    viewModelScope.launch(Dispatchers.Default) {
      ...
      setPreparing(true)
      addMessage(model = model, message = ChatMessageLoading())
      val results = com.trace.app.websearch.WebSearchClient.search(stripped)
      removeLastMessageIfLoading(model)
      ...
      val augmented =
        if (results.isEmpty()) {
          ...
          buildString {
            ...
            append(com.trace.app.websearch.WebSearchClient.formatForPrompt(results))
  ...
  private fun renderRagResponseForChat(
    response: com.trace.app.rag.RagResponse
  ): String {
...
constructor(
  ...
  ragEngine: com.trace.app.rag.RagEngine,
  memoryRepository: com.trace.app.memory.MemoryRepository,
  notificationScheduleManager: com.trace.app.notifications.NotificationScheduleManager,
) :
