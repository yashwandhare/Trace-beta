...
fun ChatViewWrapper(
  ...
  ChatView(
    ...
    onSendMessage = { model, messages ->
      ...
      val documents: MutableList<ChatMessageDocument> = mutableListOf()
      ...
      for (message in messages) {
        ...
        } else if (message is ChatMessageDocument) {
          documents.add(message)
        }
      }
      if ((text.isNotEmpty() && chatMessageText != null) || audioMessages.isNotEmpty() || documents.isNotEmpty()) {
        if (text.isNotEmpty()) {
        ...
        firebaseAnalytics?.logEvent(
          ...
          Bundle().apply {
            ...
            putBoolean("has_document", documents.isNotEmpty())
            putInt("document_count", documents.size)
    ...
    onSendVoiceMessage = { model, messages ->
      ...
      for (message in messages) {
        ...
        else if (message is ChatMessageDocument) documents.add(message)
      }
      if (text.isNotEmpty() || documents.isNotEmpty()) {
        if (text.isNotEmpty()) {
          modelManagerViewModel.addTextInputHistory(text)
        }
        viewModel.generateResponse(
