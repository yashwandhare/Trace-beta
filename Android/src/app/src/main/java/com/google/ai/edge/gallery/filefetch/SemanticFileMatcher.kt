...
object SemanticFileMatcher {
    ...
    private fun collectCandidates(context: Context): List<Pair<FileResult, Bitmap>> {
        ...
        // Lane A: recent images from MediaStore
        if (currentConfig.recentImagesCount > 0) {
            collectRecentImages(context, limit = currentConfig.recentImagesCount, seen = seenUris, out = candidates)
            Log.d(TAG, "collectCandidates: ${candidates.size} from recent images")
        }

        // Lane B: Downloads folder
        if (currentConfig.downloadsEnabled) {
        ...
        // Lane C: Screenshots folder
        if (currentConfig.screenshotsEnabled) {
            collectScreenshotsFolder(context, seen = seenUris, out = candidates)
            Log.d(TAG, "collectCandidates: ${candidates.size} total after Screenshots")
        }

        // Lane D: Documents folder
        if (currentConfig.documentsEnabled) {
            collectDocumentsFolder(context, seen = seenUris, out = candidates)
            Log.d(TAG, "collectCandidates: ${candidates.size} total after Documents")
        }
    ...
    private fun collectDownloadsFolder(
        ...
        collectFolderViaMediaStore(context, Environment.DIRECTORY_DOWNLOADS + "/", seen, out)
    }

    private fun collectScreenshotsFolder(
        context: Context,
        seen: MutableSet<String>,
        out: MutableList<Pair<FileResult, Bitmap>>,
    ) {
        collectFolderViaMediaStore(context, Environment.DIRECTORY_PICTURES + "/Screenshots/", seen, out)
    }

    private fun collectDocumentsFolder(
        context: Context,
        seen: MutableSet<String>,
        out: MutableList<Pair<FileResult, Bitmap>>,
    ) {
        collectFolderViaMediaStore(context, Environment.DIRECTORY_DOCUMENTS + "/", seen, out)
    }

    private fun collectFolderViaMediaStore(
        context: Context,
        ...
        // Using LIKE allows matching subdirectories if any, and avoids exact match requirement
        ...
        try {
            ...
            Log.e(TAG, "collectFolderViaMediaStore: query failed for path $relativePathPrefix", e)
        }
    }
